...
export const createReportMutationFn = async ({
  workspaceId,
  data,
}: createReportPayloadType): Promise<{
  message: string;
  reportId: string;
}> => {
  const response = await API.post(
    `/reports/workspace/${workspaceId}/create`,
    data
  );
  return response.data;
};

export const getReportsInWorkspaceQueryFn = async ({
  workspaceId,
}: {
  workspaceId: string;
}): Promise<Report[]> => {
  const response = await API.get(`/reports/workspace/${workspaceId}/`);
  
  // Access the 'reports' array inside response.data
...
export const deleteReportMutationFn = async ({
  workspaceId,
  reportId,
}: {
  workspaceId: string;
  ...
  const response = await API.delete(
    `/reports/workspace/${workspaceId}/delete/${reportId}`
  );
  return response.data;
};
